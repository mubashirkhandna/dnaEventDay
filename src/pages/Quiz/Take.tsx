...
import { getQuizQuestions, submitQuiz, QuizQuestion } from '../../lib/api';
...
export default function QuizTake() {
  ...
  const userEmail: string = quizUserStr ? JSON.parse(quizUserStr).email : 'anonymous';

  // Load questions from API
  useEffect(() => {
    getQuizQuestions()
      .then(setQuestions)
      .catch((e: unknown) => toast.error((e as Error).message || 'Failed to load quiz'))
  ...
  useEffect(() => {
    ...
    if (state.quizSubmissions[userEmail]) setIsSubmitted(true);
  }, [userEmail]);

  // Countdown timer synced to server quizEndTime
  useEffect(() => {
    const tick = () => {
      const state = getStore();
      if (state.quizEndTime) {
        ...
        if (remaining <= 0 && !isSubmitted) handleSubmit(answers);
      } else {
        setTimeLeft(0);
      }
    };
  ...
  const handleSubmit = async (finalAnswers = answers) => {
    if (isSubmitted || submitting || questions.length === 0) return;
    setSubmitting(true);
    const timeTaken = Math.floor((Date.now() - startTime) / 1000);
    try {
      const res = await submitQuiz(finalAnswers, timeTaken);
      setIsSubmitted(true);
      toast.success(`Quiz submitted! Your score: ${res.score} pts`);
    } catch (err: unknown) {
  ...
  const handleNext = () => {
    ...
    const newAnswers = { ...answers, [questions[currentIdx].id]: selectedOption || '' };
    setAnswers(newAnswers);
    if (currentIdx < questions.length - 1) {
      setCurrentIdx(currentIdx + 1);
      setSelectedOption(newAnswers[questions[currentIdx + 1]?.id] || null);
    } else {
    ...
    return (
      <div className="min-h-[calc(100vh-5rem)] flex items-center justify-center p-4">
        <div className="glass-card p-10 rounded-3xl text-center max-w-lg w-full border-yellow-500/30 relative overflow-hidden">
          ...
          <h2 className="text-4xl font-display font-bold text-white mb-2">Submitted!</h2>
          <p className="text-slate-300 mb-8">Thank you for participating. Wait for the admin to announce results on the main stage.</p>
          <button onClick={() => navigate('/')} className="px-8 py-3 bg-void-800 text-white rounded-xl hover:bg-void-700 transition-colors">
            Return Home
          </button>
        </div>
      </div>
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative">
      ...
      {question ? (
        <div className="glass-card p-6 md:p-10 rounded-3xl relative">
          <h3 className="text-2xl md:text-3xl font-bold text-white mb-8 leading-relaxed">{question.question}</h3>
          ...
          <div className="mt-10 pt-6 border-t border-white/10 flex justify-end">
            <button
              ...
              className={`px-8 py-4 rounded-xl font-bold transition-all flex items-center gap-2 ${!selectedOption || submitting ? 'bg-void-800 text-slate-500 cursor-not-allowed' : 'bg-yellow-500 text-black hover:bg-yellow-400 shadow-[0_0_20px_rgba(234,179,8,0.3)] transform hover:-translate-y-1'}`}
            >
              {submitting
                ? <><Loader2 className="w-4 h-4 animate-spin" /> Submitting...</>
                : currentIdx === questions.length - 1 ? 'Submit Final Answers' : 'Next Question'}
            </button>
          </div>
        </div>
      ) : (
